...
constexpr auto to_string(forward_range const & range)
  ...
  if constexpr(!std::same_as<target_string_char, std::ranges::range_value_t<forward_range>>)
    ...
    return small_vectors::basic_string<target_string_char>(std::basic_string_view(ranges::begin(range), ranges::end(range)));
  }
...
static void do_test(test_result & result)
  ...
  "stralgo_utf_left"_test = [&]
  {
    ...
    {
      constexpr auto source_u32{small_vectors::basic_fixed_string(U"🯀🯁🯂🯃🯄🯅🯆🯇🯈🯉🯊🯰🯱🯲🯳🯴🯵🯶🯷🯸🯹")};
      auto source{to_string<char_type>(source_u32)};
      constexpr auto expected_u32{small_vectors::basic_fixed_string(U"🯀🯁🯂🯃")};
...
static void do_test(test_result & result)
  ...
  "stralgo_utf_right"_test = [&]
  {
    auto fn_tmpl = []<typename char_type>(char_type const *) -> metatests::test_result
    {
      constexpr auto source_u32{small_vectors::basic_fixed_string(U"🯀🯁🯂🯃🯄🯅🯆🯇🯈🯉🯊🯰🯱🯲🯳🯴🯵🯶🯷🯸🯹")};
      auto source{to_string<char_type>(source_u32)};
      constexpr auto expected_u32{small_vectors::basic_fixed_string(U"🯶🯷🯸🯹")};
      auto expected{to_string<char_type>(expected_u32)};
      utf::utf_input_view_t view{source};
